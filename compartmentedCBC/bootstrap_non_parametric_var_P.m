...
A1=repmat([repmat(AbsoluteLevel.RuBP(1),length(rubp_MIDs_used),1);...
    ...
    repmat(AbsoluteLevel.UDPG(1),length(udpg_MIDs_used),1)],1,length(algea_c)-1);
...
A3=repmat([repmat(AbsoluteLevel.RuBP(3),length(rubp_MIDs_used),1);...
    ...
    repmat(AbsoluteLevel.UDPG(3),length(udpg_MIDs_used),1)],1,length(algea_o)-2);
...
for i=1:nboot
    ...
    for j=1:10
        ...
        [v_a1_b, ~, Chi_a1_b, ~, EXITFLAG_a1,~,P_a1_b] = QP_regression_var_P(S_a1, Data_norm_boot(algea_c,:),...
    ...
    for j=1:10
        ...
        [v_a2_b, ~, Chi_a2_b, df_a2, EXITFLAG_a2,~,P_a2_b] = QP_regression_var_P(S_a2, Data_norm_boot(algea_s,:), ...
    ...
    for j=1:10
        ...
        [v_a3_b, ~, Chi_a3_b, ~, EXITFLAG_a3,~,P_a3_b] = QP_regression_var_P(S_a3, Data_norm_boot(algea_o,:), ...
    ...
    % 
    %     [v_a4_b, ~, Chi_a4_b, ~, EXITFLAG_a4,~,P_a4_b] = QP_regression_var_P(S_a4, Data_norm_boot(algea_o_eil,:), ...
